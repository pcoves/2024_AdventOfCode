...
fn step1(input: Vec<&str>) {
    let mut total: i32 = input.iter().map(|line| count_xmas(line)).sum::<i32>();
    ...
    for row in 1..input.len() - 1 {
        for col in 1..input[0].len() - 1 {
            if 'A' == input[row].chars().nth(col).unwrap() {
                let square = [
                    &input[row - 1][col - 1..col + 2],
                    &input[row][col - 1..col + 2],
                    &input[row + 1][col - 1..col + 2],
                ];
...
fn rotate_square(square: &str) -> [char; 9] {
    [
        ...
        square.chars().nth(4).unwrap(),
        square.chars().nth(7).unwrap(),
